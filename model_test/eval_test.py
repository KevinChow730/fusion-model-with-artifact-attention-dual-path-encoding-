...
def plot_split_distributions(y_train, y_val, y_test, out_path, label_names, bins=40, density=True):
    ...
    for j in range(n_labels):
        ...
        ax.set_xlabel("值")
        ax.set_ylabel("密度" if density else "计数")
...
def plot_bland_altman(y_true_1d, y_pred_1d, out_path, title="Bland-Altman图"):
    ...
    plt.axhline(md, color="red", linestyle="--", label=f"Mean={md:.3f}")
...
def plot_correlation(y_true_1d, y_pred_1d, out_path, title="相关性"):
    y_true_1d = np.asarray(y_true_1d).ravel()
    y_pred_1d = np.asarray(y_pred_1d).ravel()
    if y_true_1d.size == 0:
        print("测试集为空，跳过绘图。")
    ...
    plt.plot(xs, k * xs + b, "r-", linewidth=1, label=f"拟合直线: y={k:.3f}x+{b:.3f}")
    plt.xlabel("参考值")
    plt.ylabel("预测值")
...
def main():
    ...
    plot_correlation(y_true[:, 0], y_pred[:, 0], os.path.join(args.out_dir, "corr_sbp.png"), title="SBP 相关性")
    plot_correlation(y_true[:, 1], y_pred[:, 1], os.path.join(args.out_dir, "corr_dbp.png"), title="DBP 相关性")
    plot_correlation(y_true[:, 2], y_pred[:, 2], os.path.join(args.out_dir, "corr_spo2.png"), title="SpO2 相关性")
    plot_correlation(y_true[:, 3], y_pred[:, 3], os.path.join(args.out_dir, "corr_hr.png"), title="HR 相关性")
