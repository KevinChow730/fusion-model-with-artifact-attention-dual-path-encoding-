...
class Trans(nn.Module):
    ...
    多任务学习：
    - 编码共享（不变）
    - 解码分支: dec_SBP / dec_DBP/ dec_HR
    - 任务头: head_SBP / head_DBP / head_HR，各输出 1
    - 最终输出: cat([SBP, DBP, HR], dim=1) -> [B, 3]
    """
    def __init__(self, input_dim=64, d_model=256, out_num=1, num_heads=16):
        ...
        # 编码（共享）
        self.enc_a = EncodeBlock(input_dim=input_dim, d_model=d_model)
        self.enc_b = EncodeBlock(input_dim=input_dim, d_model=d_model)
        ...
        # 解码分支：SBP / DBP / HR
        self.dec_SBP = DecodeBlock(input_dim=input_dim, d_model=d_model)
        self.dec_DBP = DecodeBlock(input_dim=input_dim, d_model=d_model)
        self.dec_HR = DecodeBlock(input_dim=input_dim, d_model=d_model)

        # 任务专属头（每个任务 out_num=1）
    ...
    def forward(self, x):
        ...
        # 拆分：[0,1] -> A；[2,3] -> B
        xa = x[:, 0:2, :]  # [B, 2, input_dim]
        xb = x[:, 2:4, :]  # [B, 2, input_dim]

        # 共享编码 -> [B, 1, d_model]
        a_encoded = self.enc_a(xa)
        b_encoded = self.enc_b(xb)
        ...
        attn_output = self.cross_attn(a_encoded, b_encoded)  # [B, 1, d_model]
        shared_latent = a_residual + attn_output              # [B, 1, d_model]

        # 解码分支
        dec_feat_SBP = self.dec_SBP(shared_latent)  # [B, 2, input_dim]
        dec_feat_DBP = self.dec_DBP(shared_latent)  # [B, 2, input_dim]
        dec_feat_HR = self.dec_HR(shared_latent)  # [B, 2, input_dim]

        # 任务头
        y_SBP = self.head_SBP(dec_feat_SBP)  # [B, 1]
        y_DBP = self.head_DBP(dec_feat_DBP)  # [B, 1]
        y_HR = self.head_HR(dec_feat_HR)  # [B, 1]

        # 拼接为 [B, 3]，与标签 [SBP, DBP, HR] 对齐
        y = torch.cat([y_SBP, y_DBP, y_HR], dim=1)
        return y